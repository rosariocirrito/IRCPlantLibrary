...
T_Task
*/
T_Task::T_Task(unsigned long _ms) {
    unsigned long minTime = 10L;
    PT = max(minTime, _ms);
    startTime = millis();
}
//
...
Equip::Equip(
    ...
    bool _go,
    unsigned long _setIntTmr, unsigned long _setFBTmr,
...
byte Equip::getStatusByte() {
    //
    byte bstatus = 0x00;
    if (on) {
        if (start) bstatus = 0x01;
        if (!start) bstatus = 0x03;
    }
    if (!on) {
        if (ready && !interd && !fbNok) bstatus = 0x02;
        if (cmdRem == 2) bstatus = 0x05;
        if (cmdLoc == 2) bstatus = 0x06;
        if (trm && (cmdLoc < 2)) bstatus = (byte) 18;
        if (fbNok && (cmdLoc < 2)) bstatus = (byte) 130;
        if (interd && (cmdLoc < 2)) bstatus = (byte) 64;
        if (extLock && (cmdLoc < 2)) bstatus = (byte) 32;
    }
    //  
    return bstatus;
}
...
TwinSeq::TwinSeq(String _name, Equip _eq[], unsigned long _setIntTmr, bool _bothStart, bool _bothStop, bool _seqStart) {
    name = _name;
    eq[0] = _eq[0];
    eq[1] = _eq[1];
    setIntTmr = _setIntTmr;
...
void TwinSeq::setTimerInterd(unsigned long tmr) { tmrInterd.PT = tmr; }
...
void ParallelSeq::setTimerInterd1(unsigned long tmr) { setIntTmr1 = tmr; }
void ParallelSeq::setTimerInterd2(unsigned long tmr) { setIntTmr2 = tmr; }
